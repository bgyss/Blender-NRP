"""Shared-exponent rgb9e5 HDR packing (port of the nrp reference, kept in sync).
...
[0, ~65408] with ~9 bits of relative precision on the dominant channel; negative
inputs clamp to 0, NaN encodes as 0, values above the max clamp to it.

nrp's packed cache layout stores per-segment path throughput in this format;
this module must produce bit-identical words to `nrp/rgb9e5.py` so packed caches
round-trip between the two projects.
"""
