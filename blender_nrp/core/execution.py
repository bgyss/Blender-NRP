...
class SshExecutionBackend:
    ...
    def submit(self, job: Job) -> str:
        ...
        self.runner(["ssh", self.host, "mkdir", "-p", remote_dir, remote_output])
        self.runner(["rsync", "-a", str(job_path), f"{self.host}:{remote_dir}/job.json"])
        for source, target in staged:
            self.runner(["rsync", "-a", source, f"{self.host}:{target}"])
        ...
        self.runner(["ssh", self.host, command])
    ...
    def status(self, job_id: str) -> JobProgress:
        ...
        try:
            result = self.runner(["ssh", self.host, "cat", remote["status"]])
    ...
    def fetch(self, job_id: str) -> dict[str, Path]:
        ...
        self.runner(["rsync", "-a", f"{self.host}:{remote['output']}/", f"{artifacts}/"])
    ...
    def cancel(self, job_id: str) -> None:
        ...
        if remote is None:
            return
        self.runner(["ssh", self.host, "pkill", "-f", f"{remote['dir']}/job.json"])
